# -*- coding: utf-8 -*-
"""
FreeIPA Server module (patched)

Original Author: Peter Pakos
Patched to remove cn=config dependency so that
a regular LDAP user (not cn=Directory Manager) can be used.

Original: https://github.com/wwwlde/freeipa-exporter
"""
...
class FreeIPAServer(object):
    def __init__(self, host, domain, binddn, bindpw):
        ...
        # PATCHED: use host directly instead of reading cn=config
        self._fqdn = host
        self.hostname_short = self._fqdn.replace('.%s' % domain, '')

        self._log.debug('FQDN: %s, short hostname: %s' % (self._fqdn, self.hostname_short))

        # PATCHED: derive base_dn from domain instead of reading cn=config
        ...
        # PATCHED: anon_bind and replication require cn=config access
    ...
    def _anon_bind(self):
        # PATCHED: cn=config is not accessible without Directory Manager.
        # Try to read, return N/A if access denied.
        ...
        try:
            ...
            if not results or (type(results) is list and len(results) == 0):
                self._log.debug('Cannot read cn=config (insufficient permissions), skipping anon_bind check')
                return None
            ...
            self._log.debug('anon_bind check failed: %s' % e)
            r = None
    ...
    def _replication_agreements(self):
        # PATCHED: cn=mapping tree,cn=config is not accessible without Directory Manager.
        # Try to read, return empty if access denied.
        ...
        try:
            ...
            self._log.debug('replication_agreements check failed: %s' % e)
            results = None

        if not results:
            self._log.debug('Cannot read replication agreements (insufficient permissions or no data), skipping')
            return '', True
