...
- ~~`type: php_fpm` объявлен в схеме переменных, но цикл в `tasks/main.yml` фильтрует только
  ...
  отдельной задачей, но теперь явный fail вместо тихой поломки. `/code-review` тут же поймал
  регрессию в самом фиксе: assert не учитывал `domain.enabled: false` (выключенная
  домен-заготовка с ещё не поддержанным `type` раньше молча игнорировалась обоими
  циклами-обработчиками, стала ронять play) — исправлено добавлением `when: item.enabled |
  default(true)`, симметрично гейтам `vhosts.yml`/`vhost_absent.yml` (см. ROADMAP.md, п. 41).
...
проверяет и не гарантирует~~ — **исправлено**: `tasks/main.yml` теперь явно читает `nginx.conf`
(`ansible.builtin.slurp`) и падает понятным `assert`, если в нём нет `sites-enabled`, сразу после
установки пакета/репозитория, до рендера каких-либо vhost'ов — раньше без такого include все
задеплоенные домены молча не читались бы nginx, хотя роль отработала бы «успешно».
`/code-review` уточнил проверку дважды (ROADMAP.md, п. 42): (1) исходный substring-поиск
`'sites-enabled' in content` проходил и на закомментированной строке (`# include
.../sites-enabled/*;`) — заменено на `is regex('^[^#\n]*sites-enabled', multiline=True)`,
не совпадающий, если директива закомментирована; (2) assert гарантированно падает при
`nginx_install_method: repo` — стоковый `nginx.conf` пакета `nginx.org` не подключает
`sites-enabled` вовсе (только `conf.d`), в отличие от Debian/Ubuntu-пакета (проверено
эмпирически, реальной установкой в чистом Debian 12). Это не баг проверки, а корректно
всплывший тот же риск §9.4 для второго `install_method` — `extensions/molecule/
nginx_multidomain/` проверяет только `install_method: package`, поэтому раньше не был замечен;
покрытие `repo`-метода тестами остаётся отдельной задачей. Она по-прежнему
не убирает `sites-enabled/default`: стоковый `default_server` vhost остаётся активным рядом с
доменами `nginx_multidomain`, пока его не уберут вручную или другим механизмом — это осталось
открытым (управление `default_server` — отдельное архитектурное решение, не просто проверка).
...
`nginx`, см. `ROADMAP.md`, пункт P5-31.)
