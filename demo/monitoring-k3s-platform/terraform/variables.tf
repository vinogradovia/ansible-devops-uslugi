...
variable "pool_name" {
  ...
  type        = string
  default     = "demo-monitoring-k3s-platform"
}

variable "pool_path" {
  description = "Путь на хосте для storage pool стенда"
  type        = string
  default     = "/var/lib/libvirt/images/demo-monitoring-k3s-platform"
}

# ADR-0006 §2: одна плоская libvirt-сеть (NAT).
...
variable "network_cidr" {
  description = "CIDR плоской сети стенда — отдельный от demo/mysql-ha-platform (10.66.6.0/24) и demo/postgresql-ha-platform (10.66.7.0/24), чтобы не конфликтовать при одновременном запуске"
  type        = string
  default     = "10.66.8.0/24"
}

variable "network_domain" {
  description = "DNS-домен libvirt-сети (dnsmasq), используется и в fqdn VM"
  type        = string
  default     = "monitoring-k3s-platform.demo"
}
