# ADR-0006 §4/§7: единственная VM — демонстрация роли samba_server (обычный SMB-файловый сервер,
# security = user, без docker-зависимостей) на реальном офисном сценарии: приватная шара отдела
# (запись только для своих) + публичная шара для чтения гостями. memory/vcpu — тот же лёгкий
# sizing, что у infra-dns/proxysql в demo/mysql-ha-platform (§7) — smbd/nmbd не требуют заметных
# ресурсов при таком объёме демо-данных.
#
