# ADR-0006 §4/§7: единственная VM — демонстрация reverse_proxy_npm (Nginx Proxy Manager, docker)
# + fail2ban (bundled-фильтр npm-proxy) + fail2ban_ui, ROADMAP Backlog №47. memory: 2048 — тот же
# sizing, что extensions/molecule/reverse_proxy_npm/molecule.yml (NPM в docker ощутимо тяжелее
# systemd-сервисов остальных лёгких demo-кейсов).
#
