...
47. **Новая роль `fail2ban` + реорганизация nginx/npm-демок вокруг неё.** Записано
    ...
    - **Связать роли `reverse_proxy_npm` и `nginx_multidomain`** — обе демонстрируют защиту
      публичного веб-сервиса через Fail2Ban, обе взаимоисключающие альтернативы reverse-proxy на
      одном хосте (ADR-0001 §порт-конфликт 80/443, `roles/reverse_proxy_npm/README.md`). Нужно
      решить конкретно, что значит «связать» здесь — не зафиксировано в этой сессии: (a) просто
      параллельные demo-кейсы под одной темой (fail2ban защищает и то, и другое, показывая
      взаимозаменяемость), или (b) более глубокая связь на уровне документации/ADR (общий
      документ, сравнивающий оба пути). Уточнить с пользователем перед реализацией.
    ...
    - Порядок реализации: сначала роль `fail2ban` (без неё нечего добавлять в demo-кейсы) и
      уточнение формулировки «связать npm/nginx_multidomain» с пользователем, потом
      реорганизация demo (переименование + два demo-кейса).
