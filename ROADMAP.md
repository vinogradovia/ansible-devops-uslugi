...
48. **Новая роль `backup` — централизованный сервис бекапа.** Сейчас в коллекции нет ни одной
    роли, отвечающей за резервное копирование — ни данных ролей с состоянием
    (`postgresql_replication`, `mysql_replication`, `monitoring_server`: VictoriaMetrics/Grafana/
    Loki/MinIO-данные), ни конфигов (`infra_dns` зоны, `nginx_multidomain` вхосты, `reverse_proxy_npm`
    SQLite/данные). Нужна отдельная роль `backup`, по аналогии с `fail2ban` (P5/Backlog №47) —
    независимый инфраструктурный примитив, который роли-потребители подключают под свои нужды, а
    не встраивают резервное копирование каждая по-своему.

    Открытые архитектурные вопросы, которые нужно решить перед реализацией (в духе P5, а не просто
    закодировать с ходу):
    - **Модель работы**: центральный сервер бекапа, который забирает данные с целевых хостов
      (pull, по аналогии с `monitoring_server`/`monitoring_agent`), или агент на каждом хосте,
      который сам отправляет бекап во внешнее хранилище (push)? Учитывая, что коллекция уже
      использует паттерн `_server`/`_agent` для `monitoring_*`, вероятно стоит следовать тому же
      разделению — но нужно явное решение, а не по умолчанию.
    - **Целевое хранилище**: локальный диск (демо-friendly, но не production-грade), S3-совместимое
      (MinIO уже есть в стеке `monitoring_server` — потенциальный переиспользуемый бэкенд), или
      абстракция над несколькими бэкендами (`restic`/`borg` уже умеют S3/SFTP/локально из коробки —
      кандидаты вместо велосипеда поверх `rsync`/`tar` руками).
    - **Что конкретно бекапить и как** — единого механизма нет: SQL-дампы (`pg_dump`/`mysqldump`,
      уже есть модули соответствующих коллекций — `community.postgresql`/`ansible.mysql`) для
      реплицируемых БД принципиально отличаются от бекапа файловых данных (VictoriaMetrics/Loki/
      MinIO volumes, конфиги nginx/bind9). Вероятно роль должна быть декларативной
      (список источников с типом: `postgres`/`mysql`/`files`), а не жёстко привязанной к
      конкретным ролям коллекции — тот же принцип, что `fail2ban_jails`/`fail2ban_custom_filters`
      в роли `fail2ban`.
    - **Расписание и retention**: systemd timer (согласуется с уже принятым в коллекции паттерном
      `systemd`-оркестрации, см. `monitoring_agent`/`fail2ban_ui`) + политика ротации (сколько
      бекапов хранить, GFS-схема или простое количество копий).
    - **Восстановление** — хотя бы задокументированная процедура (playbook или README), не только
      создание бекапов: непроверенный бекап не бекап.

    Демо-кейс по аналогии с `demo/nginx-multidomain`/`demo/npm` — после того как архитектура решена
    и роль реализована, а не одновременно с решением архитектурных вопросов выше.
