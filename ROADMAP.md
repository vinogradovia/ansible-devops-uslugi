...
| `reverse_proxy_traefik` | Нет molecule-сценария вообще — нерабочий каркас удалён (см. P3, пункт 27). | Собрать сценарий с нуля: реальный `converge` роли + рабочий testinfra-тест против `whoami`. |
